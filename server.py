...
def handle_login_message(conn, cmd, msg, client_id):
    """
    Handles the client's request to login to the server.
    """
    if cmd != chatlib.PROTOCOL_CLIENT['login_msg'] or msg == "":
        ...
        if is_match_and_valid_login(username, password) or is_already_logged(username, password):  # send error - for each case
            if is_already_logged(username, password):
                ...
                build_and_send_message(conn, chatlib.PROTOCOL_SERVER['login_failed_msg'], "The syntax of the message is wrong! ")
...
def handle_register_message(conn, cmd, msg, client_id):
    ...
    if cmd != chatlib.PROTOCOL_CLIENT['register_msg'] or msg == "":
        ...
        if not(is_syntax_valid(username) or is_syntax_valid(password)) or username in usernames_list:  # send error - for each case
            if username in usernames_list:
                build_and_send_message(conn, chatlib.PROTOCOL_SERVER['register_failed_msg'],
                                       "Username is taken!")
            else:
                build_and_send_message(conn, chatlib.PROTOCOL_SERVER['register_failed_msg'],
                                       "The syntax of the message is wrong!")
...
def send_client_username(conn, cmd, msg, client_id):
    ...
    """
    # if cmd != chatlib.PROTOCOL_CLIENT['send_username_msg'] or msg != "":
    #     if cmd != chatlib.PROTOCOL_CLIENT['send_username_msg']:
    #         send_error(conn, chatlib.PROTOCOL_SERVER['send_username_failed_msg'], "ERROR! The command is not send-username!")
    #     else:
    #         send_error(conn, chatlib.PROTOCOL_SERVER['send_username_failed_msg'], "ERROR! The message is not empty!")
...
def handle_see_tasks(conn, cmd, message, client_id):
    """
    Handles the client's request to see his own tasks.
    """
    if cmd != chatlib.PROTOCOL_TASKS_CLIENT['see_tasks_msg'] or message != "":
        if cmd != chatlib.PROTOCOL_SETTINGS_CLIENT['see_tasks_msg']:
            send_error(conn, chatlib.PROTOCOL_TASKS_CLIENT['see_tasks_failed_msg'], "ERROR! The command is not see-tasks!")
        else:
            send_error(conn, chatlib.PROTOCOL_TASKS_CLIENT['see_tasks_failed_msg'], "ERROR! The message is not empty!")

    else:
        whole_tasks_calendar = tb.return_part_of_tasks_by_id(data_base_connection, client_id)
        print(f"The tasks calendar of {client_id}: ", whole_tasks_calendar)

        if whole_tasks_calendar.empty:
            full_msg = no_tasks_see_msg

        else:
            full_msg = "you have tasks to see! "

        build_and_send_message(conn, chatlib.PROTOCOL_TASKS_SERVER['see_tasks_ok_msg'], full_msg)
...
def handle_tasks_calendar_message(conn, cmd, message, client_id):
    ...
    print(f"The tasks calendar of {client_id}: \n")
    print(tasks_calendar)

    if tasks_calendar.empty:
        msg_part_1 = no_tasks_see_msg
...
def handle_shared_diaries_outgoing_requests_msg(conn, cmd, message, client_id):
    ...
    login_dict = create_login_dictionary("id")
    client_username = login_dict[client_id][0]
    all_requesters = tb.list_of_values(cursor, diaries_sharing_requests_table, "addresser")  # the user has friends

    if client_username not in all_requesters:  # the user havn't sent any request yet
...
def handle_new_share_diary_request(conn, cmd, message, client_id):
    ...
    if cmd != chatlib.PROTOCOL_SHARED_DIARIES_CLIENT['new_share_diary_request_msg'] or message == "":
        ...
        if usernames_list != []:
            ...
            is_request_ok = True
            if "groups" in info_dictionary.keys():
                if info_msg in info_dictionary["groups"]:
                    is_request_ok = False
            ...
            if is_request_ok:
                ...
                if "groups" not in info_dictionary.keys():
                    info_dictionary["groups"] = []
                info_list_share_diary_requests = tb.list_of_values(cursor, diaries_sharing_requests_table, "info")
                info_list_share_diary_groups = tb.list_of_values(cursor, tasks_table, "info")

                # info_dictionary["groups"].append(info_msg)
                list_of_info = []
                for info in info_list_share_diary_requests:
                    list_of_info.append(info)

                for info in info_list_share_diary_groups:
                    list_of_info.append(info)

                info_dictionary["groups"] = list_of_info
    ...
    if cmd != chatlib.PROTOCOL_SHARED_DIARIES_CLIENT['reject_share_diary_request_msg'] or message == "":
        ...
        if specific_request is None:
            ...
            if client_username in recipients_list:
                ...
                tb.change_recipients_situation(data_base_connection, request_info, new_recipients)
    ...
    if cmd != chatlib.PROTOCOL_SHARED_DIARIES_CLIENT['see_specific_shared_diaries_group_msg'] or message == "":
        ...
        if specific_group is None:
            ...
            if client_username in group_recipients or client_username == group_manager:
                ...
                # sorting
                year_sorted_shared_diary = shared_diary.sort_values("year")
                month_sorted_shared_diary = year_sorted_shared_diary.sort_values("month")
                sorted_shared_diary = month_sorted_shared_diary.sort_values("day")
...
def handle_client_message(conn, cmd, message, client_id):
    ...
    if latest_cmd not in chatlib.PROTOCOL_CLIENT.values() and latest_cmd not in chatlib.PROTOCOL_SETTINGS_CLIENT.values() and latest_cmd not in chatlib.PROTOCOL_SEARCH_CLIENT.values() and latest_cmd not in chatlib.PROTOCOL_TASKS_CLIENT.values() and latest_cmd not in chatlib.PROTOCOL_FRIENDSHIP_REQUESTS_CLIENT.values() and latest_cmd not in chatlib.PROTOCOL_PROFILE_CLIENT.values() and latest_cmd not in chatlib.PROTOCOL_SHARED_DIARIES_CLIENT.values():
        ...
        # elif latest_cmd == chatlib.PROTOCOL_TASKS_CLIENT['see_tasks_msg']:
        #     handle_see_tasks(conn, latest_cmd, message, client_id)
            ...
            handle_send_username_data(conn, latest_cmd, message, client_id)  # change username_data_msg-> 4 variations
...
def handle_after_user_deleted(client_username):
    """
    Handles the consequences of the user being deleted.
    """
    global hobbies_dictionary
    global friendships_requests_dictionary
    global friendships_dictionary
    ...
    if client_username in hobbies_dictionary.keys():
        del hobbies_dictionary[client_username]
        save_hobbies_dictionary(hobbies_dictionary)

    # did anyone send friendship request to him?

    if client_username in friendships_requests_dictionary.keys():
        del friendships_requests_dictionary[client_username]
        save_friendships_requests_dictionary(friendships_requests_dictionary)

    # did he send friendship requests?

    for key in friendships_requests_dictionary.keys():
        if client_username in friendships_requests_dictionary[key]:
            friendships_requests_dictionary[key].remove(client_username)
            save_friendships_requests_dictionary(friendships_requests_dictionary)

    # did he have friends?

    if client_username in friendships_dictionary.keys():
        for user in friendships_dictionary[client_username]:
            friendships_dictionary[user].remove(client_username)
            save_friendships_dictionary(friendships_dictionary)
...
def ceaser_cypher(s, k):
    # ceaser cypher code - server side.
    # k = key, f = string
