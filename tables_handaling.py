...
def create_tasks_sharing_requests_table(conn):
    """
    Creates the table, first the function creates a cursor that can change or edit the table.
    Then the function creates the table if it doesn't exist already, with specific columns.
    ...
    try:
        conn.execute('''CREATE TABLE IF NOT EXISTS tasks_sharing_requests (
                            group_info TEXT,
                            request_originator TEXT,
                            day TEXT,
                            month TEXT,
                            year TEXT,
                            info TEXT,
                            approvals_amount TEXT,
                            users_who_approved TEXT
                        )''')
...
def create_tasks_sharing_table(conn):
    """
    Creates the table, first the function creates a cursor that can change or edit the table.
    Then the function creates the table if it doesn't exist already, with specific columns.
    ...
    try:
        conn.execute('''CREATE TABLE IF NOT EXISTS tasks_sharing (
                            group_info TEXT,
                            day TEXT,
                            month TEXT,
                            year TEXT,
...
def update_tasks_sharing_requests_table(conn, values_list):
    """
    updates the diaries sharing requests table.
    :param conn: Connection object
    :param values_list: the lict of the new values
...
def update_tasks_sharing_table(conn, values_list):
    """
    updates the diaries sharing requests table.
    :param conn: Connection object
    :param values_list: the lict of the new values
    ...
    try:
        ...
        query = "INSERT INTO diaries_sharing VALUES (?, ?, ?, ?, ?, ?)"
        cursor.execute(query, values_list)
...
def change_recipients_situation(conn, info_data, new_recipients):
    """
    Change the approval situation in the specific row in a specific index.
    index - the index in the list of the current approvals to change
    """
...
# def change_opposers_and_reasons(conn, info_data, client_username, reason_why):
#     """
#     Change the approval situation in the specific row (in a specific index).
#     index - the index in the list of the current approvals to change
#     """
#     try:
#         cursor = conn.cursor()
#         query = "SELECT opposers, objection_reasons FROM diaries_sharing_requests WHERE info = ?"
#         cursor.execute(query, info_data)
#
#         rows = cursor.fetchall()[0]
#         opposers = rows[0]
#         objection_reasons = rows[1]
#
#         # change opposers
#
#         if str(opposers) == "":
#             new_opposers = client_username
#
#         else:
#             new_opposers = opposers + ", " + client_username
#
#         # change objection reasons
#
#         if str(objection_reasons) == "":
#             new_objection_reasons = reason_why
#
#         else:
#             new_objection_reasons = objection_reasons + ", " + reason_why
#
#         print(new_objection_reasons)
#
#         # update
#         first_update_query = "UPDATE diaries_sharing_requests SET opposers = ? WHERE info = ?"
#         first_update_values = (new_opposers,) + tuple(info_data) + (opposers, objection_reasons)
#         cursor.execute(first_update_query, first_update_values)
#
#         second_update_query = "UPDATE diaries_sharing_requests SET objection_reasons = ? WHERE info = ?"
#         second_update_values = (new_objection_reasons,) + tuple(info_data) + (new_opposers, objection_reasons)
#         cursor.execute(second_update_query, second_update_values)
#
#         # Commit the changes
#         conn.commit()
#
#         return new_opposers, new_objection_reasons
#
#     except Exception as e:
#         print(e)
...
def change_task_sharing_requests_table(conn, column, val, task_info):
    """
    Changes the val in the known_row in the task_sharing_requests table.
    """
    try:
        cursor = conn.cursor()

        query = f"UPDATE task_sharing_requests SET {column} = ? WHERE info = ?"
        cursor.execute(query, [val] + task_info)
...
def delete_tasks_sharing_requests_row(conn, task_info):
    """
    Deletes a row of the tasks sharing requests table.
    """
    try:
        cursor = conn.cursor()
        query = "DELETE FROM tasks_sharing_requests WHERE info = ?"
        cursor.execute(query, task_info)
        conn.commit()
...
def return_users_row(conn, client_id):
    ...
    try:
        cursor = conn.cursor()
        query = f"SELECT * FROM users WHERE id = '{client_id}'"
...
def return_name_id_by_username(conn, username):
    ...
    try:
        cursor = conn.cursor()
        query = f"SELECT name, id FROM users WHERE username = '{username}'"
        cursor.execute(query)
...
def return_part_of_tasks_by_id(conn, client_id):
    """
    Returns a specific part of tasks - not all!
    :param conn: A connection to the database.
    :param client_id: An id of existing client.
    :return:
    """
    ...
    try:
        cursor = conn.cursor()
        query = f"SELECT day, month, year, info, did_finish FROM tasks WHERE client_id = '{client_id}'"
        df = pd.read_sql_query(query, conn)
...
def change_shared_diaries_tables(conn, client_username):
    """
    Returns all the ingoing requests to the client conn (his username is client_username)
    """
    ...
    try:
        cursor = conn.cursor()
        query = f"SELECT addresser, recipients FROM diaries_sharing_requests WHERE addresser = {client_username} OR ', ' || recipients || ', ' LIKE '%" + client_username + "%';"
        df = pd.read_sql_query(query, conn)
...
def return_all_share_diary_groups_containing_client(conn, client_username):
    """
    Returns all groups that invovles the client (username = client_username)
    """
...
# tasks sharing requests


def return_tasks_sharing_requests_row(conn, info_msg):
    """
    Returns a tasks sharing requests row. None if it doesn't exist.
    (by task info)
    """
    try:
        query = "SELECT * FROM tasks_sharing_requests WHERE info = ?"
        df = pd.read_sql_query(query, conn, params=[info_msg])
...
def return_all_group_tasks_sharing_requests(conn, info_data):
    """
    Returns a specific part of tasks_sharing_requests_table
    :param conn: A connection to the database.
    :param info_data: the info of the group.
    :return: A pandas DataFrame containing the selected rows, or None if no rows are found.
    """
    try:
        query = f"SELECT request_originator, day, month, year, info, approvals_amount FROM tasks_sharing_requests WHERE group_info = ?"
        df = pd.read_sql_query(query, conn, params=[info_data])
        if df.empty:
            return None
        else:
            return df
...
def return_task_sharing_row(conn, info_msg):
    """
    Returns a task sharing row. None if it doesn't exist.
    """
    try:
        query = "SELECT * FROM task_sharing WHERE info = ?"
        df = pd.read_sql_query(query, conn, params=[info_msg])
...
def return_all_group_shared_tasks(conn, info_data):
    """
    Returns a specific part of tasks_sharing_requests_table
    :param conn: A connection to the database.
    :param info_data: the info of the group.
    ...
    try:
        query = f"SELECT day, month, year, info, did_finish FROM tasks_sharing WHERE group_info = ?"
        df = pd.read_sql_query(query, conn, params=[info_data])
        if df.empty:
            return None
        else:
...
def main():
    ...
    cursor.execute("DELETE from diaries_sharing_requests")
    cursor.execute("DELETE from diaries_sharing_groups")
    ...
    create_tasks_sharing_requests_table(data_base_connection)
    create_tasks_sharing_table(data_base_connection)
