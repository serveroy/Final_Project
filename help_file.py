...
logo_path = 'C:\cyber_project_real\project_logo.png'
hobbies_picture_path = 'C:\cyber_project_real\hobby-picture.png'
search_picture_path = 'C:\cyber_project_real\search-curr-picture.jpg'
...
class MyScreenManager(ScreenManager):
    def __init__(self, conn, **kwargs):
        ...
        self.add_widget(SeeUserDataScreen(conn=conn, name="see_user_data_screen"))  # sub screen 2 - search by hobbies
...
class SeeUserDataScreen(Screen):
    ...
    def on_approve_friendship_request_press(self, instance):
        ...
        elif curr_cmd == chatlib.PROTOCOL_FRIENDSHIP_REQUESTS_SERVER['approve_friendship_request_ok_msg']:
...
class SeeTasksScreen(Screen):
    ...
    def on_choose_task_button_press(self, instance):
        ...
        if not index.isdigit():
            ...
            if 0 <= real_index < len(self.tasks_table.row_data):
                # Index is in the valid range
                row_data = self.tasks_table.row_data[real_index]
    ...
    def on_enter(self):
        ...
        if screen_flow["prev"] == "tasks_screen" or screen_flow["prev"] == "see_specific_task_screen":
            ...
            if curr_cmd == chatlib.PROTOCOL_TASKS_SERVER['see_tasks_calendar_ok_msg']:
                tasks_msg = msg.split("#")[0]

                if tasks_msg != no_tasks_see_msg:
                    # first design
                    ...
                    layout.add_widget(MDRaisedButton(text="Choose task", size_hint=(0.2, 0.6), font_size=self.width / 40,
                    ...
                    self.box_layout.add_widget(self.tasks_table)

                else:
                    self.message_label.text = tasks_msg
...
class EditSpecificTaskScreen(Screen):
    ...
    def on_see_specific_task_back_button_press(self, instance):
        """
        Going back to the hobbies screen.
        """
        self.edit_the_task_response_label.text = ''
        self.info_input.text = ''
        self.date_input.text = ''
    ...
    def on_edit_the_task_button_press(self, instance):
        ...
        whole_task_str = ""
        for i in range(len(whole_task)):
            whole_task_str += whole_task[i]
            if i < len(whole_task) - 1:
                whole_task_str += "^"
    ...
    def on_enter(self):
        """
        Shows the client the specific task
        :return:
        """
        global screen_flow
        tasks_first_dict = screen_flow["specific task to see"]
...
class SharedDiariesScreen(Screen):
    ...
    def on_enter(self):
        """
        A function that will run each time the class in being entered.
        """
        # first design
        # add the image at the top
        self.add_widget(Image(source=logo_path, size_hint=(0.25, 0.25), pos_hint={'center_x': 0.15, 'top': 0.9}))
...
class OutgoingShareDiaryRequestsScreen(Screen):
    def __init__(self, conn, name="outgoing_share_diary_requests_screen", **kwargs):
        ...
        friends_layout = BoxLayout(orientation='horizontal', spacing=40, size_hint=(0.7, 0.2),
        ...
        friends_layout.add_widget(choose_friends_button)
        self.add_widget(friends_layout)
    ...
    def move_to_send_share_diary_request_screen(self):
        """
        Moving back to the send share diary request screen.
        """
        self.message_label.text = ''
        self.friends_input.text = ''
        self.friends_tables_case1_layout.clear_widgets()
    ...
    def on_choose_friends_button_press(self, instance):
        """
        The client chose an index and want to see this task.
        """
        global screen_flow
...
class SendShareDiaryRequestScreen(Screen):
    ...
    def on_enter(self):
        # create the usernames message label widget
    ...
    def on_send_share_diary_request_button_press(self, instance):
        ...
        if not val_date(max_date):
            self.message_label.text = "ERROR! The date you entered in invalid! "
...
class IngoingShareDiaryRequestsScreen(Screen):
    ...
    def move_to_see_specific_ingoing_share_diary_request_screen(self):
        """
        Moving back to the delete hobby screen.
        """
        self.clear_widgets()
...
def val_date(date):
    """
    Checks if the input (date) is in the correct date form (DD/MM/YYYY)  # update!  (maybe?)
    """
    from datetime import datetime
    try:
...
def reverse_ceaser_cypher(f, k):
    # reverse ceaser cypher code - client side
    # k = key, f = string
    ...
    for i in range(len(f)):
        c = f[i]
        if c.isalpha():
            # Shift the character back by k positions
...
def get_key(conn):
    ...
    Then, using the server's public key, it is calculate the server and client's shared key.
    """
    global client_key  # the client_key (=shared key) is a globel variable
